...
const Carousel = () => {
    const images = [Coffee1, Coffee2, Coffee3, Coffee4, Coffee5, Coffee6, Coffee7, Coffee8, Coffee9, Coffee10, Coffee11, Coffee12, Coffee13]
        ...
        <React.Fragment>
            <Slider {...settings}>
                {images.map((image, index) => (
                    <div key={index} className='relative h-72 overflow-hidden rounded-lg md:h-96'>
                        <img src={image} alt={`Coffee ${index + 1}`} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" />
                    </div>
                ))}
            </Slider>
        </React.Fragment>
